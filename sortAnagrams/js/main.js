...
Array.prototype.shuffle = function(){
    for( var i=0, r=0,l=this.length,tmp=null; i<l; i++ ){
        tmp = this[i];
        r=randRange(0, l-1);
        this[i] = this[r];
        this[r]=tmp;
...
//Group anagrams
function sort( aAnagrams ){
    var hash = {},
        key   = '';
    for( var ana in aAnagrams ){
        if( aAnagrams.hasOwnProperty(ana) ){
            key = aAnagrams[ana].split('').sort().join('');
            if( hash[key] ){
                hash[key].push(aAnagrams[ana]);
            }else{
                hash[key] = [aAnagrams[ana]];
            }
        }
    }
    ...
    var out = [];
    for( var key in hash ){
        out = out.concat(hash[key]);
    }
    return out;
}

console.log( sort(aAnas) );
